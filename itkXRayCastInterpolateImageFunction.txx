...
public:
   ...
   * \return True if a valid ray was specified.
   */
  bool IntegrateXRay(double &integral);
...
void
XRayCastHelper<TInputImage, TCoordRep>
::Initialise(void)
{
...
/* -----------------------------------------------------------------------
   IntegrateXRay() - Integrate intensities above a threshold.
   ----------------------------------------------------------------------- */

template<class TInputImage, class TCoordRep>
bool
XRayCastHelper<TInputImage, TCoordRep>
::IntegrateXRay(double &integral)
{
//  double posn3D_x, posn3D_y, posn3D_z;

  const double xrayAbsorptionCoeffAir = 1.184e-6;
  const double xrayAbsorptionCoeffWater = 995.65e-6;
  ...
  for (m_NumVoxelPlanesTraversed=0;
    ...
    {
      double HU =  this->GetCurrentIntensity();
//      if (HU > 200) HU = lastHU;
      double xrayAbsorptionCoeff =  (xrayAbsorptionCoeffWater - xrayAbsorptionCoeffAir) * HU / 1000.0 + xrayAbsorptionCoeffWater;
      if (xrayAbsorptionCoeff < 0) xrayAbsorptionCoeff = 0;
      integral  *= std::exp( xrayAbsorptionCoeff * -1 * this->GetRayPointSpacing() );
 ...
 *
 *
 * Rest of this code is the actual XsRayCastInterpolateImageFunction
 * class
 *
 *
 **************************************************************************/
...
template<class TInputImage, class TCoordRep>
XRayCastInterpolateImageFunction< TInputImage, TCoordRep >
::XRayCastInterpolateImageFunction()
{
  m_Threshold = 0.;
...
template<class TInputImage, class TCoordRep>
typename XRayCastInterpolateImageFunction< TInputImage, TCoordRep >
::OutputType
XRayCastInterpolateImageFunction< TInputImage, TCoordRep >
::Evaluate( const PointType& point ) const
{
  ...
  XRayCastHelper<TInputImage, TCoordRep> ray;
  ray.SetImage( this->m_Image );
  ray.ZeroState();
  ray.Initialise();

  ray.SetRay(point, direction);
  ray.IntegrateXRay(integral);
